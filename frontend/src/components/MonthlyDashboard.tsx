    ...
    <Card className="border-2 shadow-xl bg-gradient-to-br from-slate-100 to-gray-200 dark:from-slate-900 dark:to-gray-900">
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4 md:grid-cols-3">
          <div className="p-4 rounded-lg bg-blue-100 dark:bg-blue-950/30 border-2 border-blue-300 dark:border-blue-800">
          ...
          <div className="p-4 rounded-lg bg-green-100 dark:bg-green-950/30 border-2 border-green-300 dark:border-green-800">
            <p className="text-sm text-muted-foreground mb-1">Total Amount</p>
            <p className="text-3xl font-bold text-green-600 dark:text-green-400">₹{data?.totalAmount?.toFixed(2) || 0}</p>
          </div>
          <div className="p-4 rounded-lg bg-purple-100 dark:bg-purple-950/30 border-2 border-purple-300 dark:border-purple-800">
            <p className="text-sm text-muted-foreground mb-1">Avg per Day</p>
            <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">
              {data?.totalMeals ? (data.totalMeals / new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 0).getDate()).toFixed(1) : 0}
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {data?.byType && Object.entries(data.byType).map(([type, count]: [string, any]) => (
              <div key={type} className="flex items-center justify-between p-3 rounded-lg bg-gray-300 dark:bg-gray-800">
        ...
        <div>
          ...
          <div className="space-y-2">
            {data?.byWeek && Object.entries(data.byWeek).map(([week, stats]: [string, any]) => (
              <div key={week} className="flex items-center justify-between p-3 rounded-lg bg-gray-300 dark:bg-gray-800">
